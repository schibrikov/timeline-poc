...
function fetchUpdatedObservableEmployees(date: Date) {
  return getDataForDate(date).then(data => JSON.parse(data));
}
...
const timeline = (store: Store<StoreState>) => {
  store.on('@init', () => {
    ...
    fetchUpdatedObservableEmployees(now).then(employees =>
      store.dispatch('timeline/data', employees)
    );
  ...
  store.on(
    ...
    ({ employees }, { employeeId, periodId, period }: UpdateAction) => {
      const targetEmployeeIndex = employees.findIndex(
        (emp: Employee) => emp.id === employeeId
      );
      const targetPeriodIndex = employees[
        targetEmployeeIndex
      ].periods.findIndex((period: Period) => period.id === periodId);
